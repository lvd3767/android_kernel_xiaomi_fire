...
static void rfx_get_util(struct rfx_cpu *rfx_c)
{
	struct rq *rq = cpu_rq(rfx_c->cpu);
	unsigned long util_cfs = cpu_util_cfs(rq);
	unsigned long max = arch_scale_cpu_capacity(NULL, rfx_c->cpu);

	rfx_c->bw_min = cpu_bw_dl(rq);
	rfx_c->bw_max = max;
	rfx_c->util = schedutil_cpu_util(rfx_c->cpu, util_cfs, max,
					  FREQUENCY_UTIL, NULL);
}
